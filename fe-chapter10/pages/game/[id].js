...
export default function Game() {
  ...
  const [isMyTurn, setIsMyTurn] = useState((userId === room.hostUserId && (room.turn - 1) % 2 === 0) || (userId === room.guestUserId && (room.turn - 1) % 2 === 1));

  const currentRound = Math.floor((room.turn - 1) / 2) + 1;
  const isUserWin = (userId === room.hostUserId && room.isHostWinRound) || (userId === room.guestUserId && room.isGuestWinRound);
  ...
  const handleHostSelection = async (hostSelection) => {
    ...
    await setIsMyTurn((userId === updatedRoom.hostUserId && (updatedRoom.turn - 1) % 2 === 0) || (userId === updatedRoom.guestUserId && (updatedRoom.turn - 1) % 2 === 1));
    ...
    <div className="bg-gray-100 h-screen w-full py-4 px-8 flex flex-col">
      ...
      <div className="flex grow justify-between mt-8">
        ...
        <div className="flex flex-col justify-between h-full items-center">
          ...
          {
            isRoundFinished ?
              <div className={`px-8 py-10 rounded-2xl text-center ${isUserWin ? "bg-green-200" : "bg-red-200"}`}>
                <p className="text-5xl">{isUserWin ? "You Win" : "You Lose"}<br />This Round!</p>
              ...
              :
              <p className="text-4xl text-slate-500 text-center">{isMyTurn ? "Your" : "Enemy's"} Turn</p>
          }
          <button
