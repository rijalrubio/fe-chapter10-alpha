...
export default function SinglePlayer() {
  ...
  const handleHostSelection = async (playerSelection) => {
    ...
    if (playerSelection === COMSelection) {
      ...
      if ((playerSelection === 1 && COMSelection === 3) || (playerSelection === 2 && COMSelection === 1) || (playerSelection === 3 && COMSelection === 2)) {
        ...
        <div className="flex flex-col justify-between h-full items-center">
          <div />
          {
            room.result !== undefined ?
              <div className={`px-8 py-10 rounded-2xl text-center ${room.result === "win" ? "bg-green-200" : room.result ==="draw" ? "bg-gray-200" : "bg-red-200"}`}>
                <p className="text-5xl">{room.result === "win" ? "You Win" : room.result ==="draw" ? "Draw" : "You Lose"}</p>
